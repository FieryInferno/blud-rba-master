                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Data Pemetaan Sub Kegiatan</h4>
                        </div>
                        <div class="card-body">
                            ...
                            <thead>
                                ...
                                <th>Kode Sub Kegiatan BLUD</th>
                                <th>Nama Sub Kegiatan BLUD</th>
                                <th>Kode Sub Kegiatan APBD</th>
                                <th>Nama Sub Kegiatan APBD</th>
                                @if (Auth::user()->hasRole('admin'))
                                <th>Opsi</th>
                                @endif
                                </tr>
                            ...
                            <tbody>
                                @foreach ($mapSubKegiatan as $item)
                                <tr>
                                    <td>{{ $item->kodeUnitKerja }}</td>
                                    <td>{{ $item->unitKerja->nama_unit }}</td>
                                    <td>{{ $item->subKegiatanBlud->kodeSubKegiatan }}</td>
                                    <td>{{ $item->subKegiatanBlud->namaSubKegiatan }}</td>
                                    <td>{{ $item->subKegiatanApbd->kodeSubKegiatan }}</td>
                                    <td>{{ $item->subKegiatanApbd->namaSubKegiatan }}</td>
                                    @if (Auth::user()->hasRole('admin'))
                                    <td>
                                    <button class="btn btn-sm btn-warning btn-edit"
                                        ...
                                        data-nama-unit="{{ $item->unitKerja->nama_unit }}"
                                        data-kode-blud="{{ $item->kodeSubKegiatan }}"
                                        data-nama-blud="{{ $item->subKegiatanBlud->namaSubKegiatan }}"
                                        data-kode-apbd="{{ $item->kodeSubKegiatanApbd }}"
                                        data-nama-apbd="{{ $item->subKegiatanApbd->namaSubKegiatan }}">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Buat Pemetaan Sub Kegiatan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.mapSubKegiatan.store') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label>Kode Unit Kerja</label>
                        <select name="kodeUnitKerja" class="form-control selectpicker" data-live-search="true" title="Pilih Kode">
                    ...
                    <div class="form-group">
                        <label>Kode Sub Kegiatan BLUD</label>
                        <select name="kodeSubKegiatanBlud" class="form-control selectpicker" title="Pilih Kode">
                            @foreach ($subKegiatan as $item)
                                <option value="{{ $item->kodeSubKegiatan }}">{{ $item->kodeSubKegiatan }} - {{ $item->namaSubKegiatan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Kode Sub Kegiatan APBD</label>
                        <select name="kodeSubKegiatanApbd" class="form-control selectpicker" title="Pilih Kode">
                            @foreach ($subKegiatan as $item)
                                <option value="{{ $item->kodeSubKegiatan }}">{{ $item->kodeSubKegiatan }} - {{ $item->namaSubKegiatan }}</option>
                            @endforeach
                        </select>
...
<div class="modal fade" tabindex="-1" role="dialog" id="editModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Sunting Pemetaan Sub Kegiatan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.mapSubKegiatan.update') }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select id="edit-kode-blud" name="kegiatan_id_blud" class="form-control selectpicker" title="Pilih Kode">
                            @foreach ($subKegiatan as $item)
                                <option value="{{ $item->idSubKegiatan }}">{{ $item->kodeSubKegiatan }} - {{ $item->namaSubKegiatan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
