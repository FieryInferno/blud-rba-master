              ...
              <div class="col-md-12">
                  <div class="card">
                      <div class="card-header">
                        <h4>Salin Anggaran untuk RBA</h4>
                      </div>
                      <div class="card-body">
                        <form class="form-inline">
                          ...
                          <a href="{{ route('admin.salinanggaran.rba') }}" class="btn btn-outline-danger mb-2"><i class="fa fa-reset"></i> Reset</a>
                        </form>
                        <form action="{{ route('admin.salinanggaran.rba') }}" method="POST" id="form-copy">
                        @csrf
                        <table class="table table-bordered table-hover">
                          ...
                          <tbody>
                            @foreach ($rba as $item)
                              <tr>
                                <td><input type="checkbox" name="rba_selected[]" value="{{ $item->id }}"></td>
                                <input type="hidden" name="rba[]" value="{{ $item->id }}">
...
<script>
  $(document).ready(function () {
    ...
    $("#form-copy").on('submit', function(e) {
      e.preventDefault();
      var rbaLength = $("input[type='checkbox'][name='rba_selected[]']:checked").length;
      if(!rbaLength) {
