...
<div class="main-content">
  <section class="section">
      ...
      <form action="{{ route('admin.rba1.update_pak', $rba->id) }}" method="POST" id="form-rba">
        @csrf
        @method('PUT')
    ...
    $(document).ready(function () {
      ...
      $('#form-rba').on('submit', function (e) {
       ...
       var formData = $(this).serialize();
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.rba1.update_pak', $rba->id) }}",
          data: formData,
          success:function(response){
            iziToast.success({
              title: 'Sukses!',
          ...
          sumberDana.forEach(function (item) {
            $('.table-sumber-dana tbody').append(`
              <tr>
                ...
                <td><input type="text" name="nominal_pak[]" value="${formatCurrency(item.jumlahPAK)}" class="form-control" readonly></td>
              </tr>
            `);
          });
