...
<div class="main-content">
  <section class="section">
      ...
      <form action="{{ route('admin.rba31.update_pak', $rba->id) }}" method="POST" id="form-rba">
        @csrf
        @method('PUT')
                                        ...
                                        @foreach ($anggaran as $index => $item)
                                          @if ($index == 0)
                                            ...
                                            <tr>
                                              ...
                                              <td><input type="text" name="uraian[]" class="form-control" readonly value="{{ $item->uraian }}"></td>
                                              ...
                                              <td><input type="text" name="tarif[]" class="form-control money" readonly onkeyup="typingTarif(event)" value="{{ $item->tarif }}"></td>
                                              ...
                                              <td><input type="text" name="tarif_pak[]" class="form-control money" onkeyup="typingTarifPak(event)" value="{{ $item->tarif_pak }}"></td>
    ...
    $(document).ready(function () {
      ...
      $('#form-rba').on('submit', function (e) {
       ...
       var formData = $(this).serialize();
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.rba31.update_pak', $rba->id) }}",
          data: formData,
          success:function(response){
            iziToast.success({
              title: 'Sukses!',
                  ...
                  response.data.rincian.forEach(function (itemRincian) {
                    if (item.kode_akun == itemRincian.kode_akun) {
                      obj.push({
                        ...
                        'Jumlah Murni': `<input type="text" name="jumlah[]" class="form-control" value="${formatCurrency(itemRincian.volume * itemRincian.tarif)}" readonly>`,
                        'Volume PAK': `<input type="text" name="volume_pak[]" class="form-control" onkeyup="typingVolumePak(event)" value="${itemRincian.volume_pak}">`,
                        'Satuan PAK': `<input type="text" name="satuan_pak[]" class="form-control" value="${itemRincian.satuan_pak}" readonly>`,
                        ...
                        'Jumlah PAK': `<input type="text" name="jumlah_pak[]" class="form-control" value="${formatCurrency(itemRincian.volume_pak * itemRincian.tarif_pak)}" readonly>`,
      ...
      $('.table-rba tbody').on('click', '.btn-add', function () {
        ...
        let element = `
            <tr class="text-dark">
              ...
              <td>
                <input type="text" name="tarif[]" class="form-control money" readonly onkeyup="typingTarif(event)" value="0">
              </td>
              <td>
                <input type="text" name="jumlah[]" class="form-control" value="0" readonly>
              </td>
              <td>
