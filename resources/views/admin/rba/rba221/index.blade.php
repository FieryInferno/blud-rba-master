...
<div class="main-content">
  <section class="section">
      <div class="section-header">
          <h1>RBA Belanja - {{ auth()->user()->statusAnggaran->status_anggaran }}</h1>
      </div>
      ...
      <div class="section-body">
          <div class="row">
            ...
            @if (auth()->user()->statusAnggaran->status_perubahan == 'MURNI')
              <a class="btn btn-primary mb-4 ml-3" href="{{ route('admin.rba2.create') }}">
                  ...
                  <div class="card">
                      ...
                      <div class="card-body">
                        ...
                        <form class="form-inline">
                          ...
                          <a href="{{ route('admin.rba2.index') }}" class="btn btn-outline-danger mb-2"><i class="fa fa-reset"></i> Reset</a>
                        </form>
                          ...
                          <tbody>
                            @foreach ($rba as $item)
                              <tr>
                                ...
                                @endif
                                <td>{{ $item->mapSubKegiatan ? $item->mapSubKegiatan->subKegiatanBlud->namaSubKegiatan : '-' }}</td>
                                <td>{{ $item->created_at->diffForHumans() }}</td>
                                <td>
                                  @if (auth()->user()->statusAnggaran->status_perubahan == 'MURNI')
                                    <a href="{{ route('admin.rba221.edit', $item->id) }}" class="btn btn-sm btn-primary">
                                      <span class="fa fa-plus"></span> Pilih
                                    </a>
                                  @else
                                    <a href="{{ route('admin.rba221.edit_pak', $item->id) }}" class="btn btn-sm btn-primary">
                                      <span class="fa fa-plus"></span> Pilih
                                    </a>
                                  @endif
                                  ...
                                  @else
                                    ...
                                    @endif
                                  @endif
                                  <a class="btn btn-sm btn-warning" href="{{ route('admin.report.rba', $item->id) }}" target="_blank">
...
<form id="form-delete" action="{{ route('admin.rba2.destroy') }}" class="d-none" method="POST">
  @csrf
  @method('DELETE')
