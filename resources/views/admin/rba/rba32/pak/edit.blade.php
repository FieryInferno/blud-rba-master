...
<div class="main-content">
  <section class="section">
      ...
      <form action="{{ route('admin.rba32.update_pak', $rba->id) }}" method="POST" id="form-rba">
        @csrf
        @method('PUT')
                                  ...
                                  <tbody>
                                      ...
                                      @foreach ($rba->rincianAnggaran->groupBy('akun_id') as $key => $anggaran)
                                        @foreach ($anggaran as $index => $item)
                                          @if ($index == 0)
                                            <tr>
                                              <td><button type="button" class="btn btn-add btn-sm btn-primary is-parent parent-item d-none" disabled><i class="fas fa-plus"></i></button></td>
                                            ...
                                            <tr>
                                              <td><button type="button" class="btn btn-remove btn-sm btn-danger d-none" disabled><i class="fas fa-minus"></i></button></td>
                                              ...
                                              <td><input type="text" name="volume[]" class="form-control" readonly onkeyup="typingVolume(event)" value="{{ $item->volume }}"></td>
                                              <td><input type="text" name="satuan[]" class="form-control" readonly value="{{ $item->satuan }}"></td>
                                              <td><input type="text" name="tarif[]" class="form-control" readonly onkeyup="typingTarif(event)" value="{{ $item->tarif }}"></td>
                                              <td><input type="text" name="jumlah[]" class="form-control" readonly value="{{ ($item->volume * $item->tarif) }}"></td>
                                              <td><input type="text" name="volume_pak[]" class="form-control" onkeyup="typingVolumePak(event)" value="{{ $item->volume_pak }}"></td>
                                              <td><input type="text" name="satuan_pak[]" class="form-control" value="{{ $item->satuan_pak }}"></td>
                                              <td><input type="text" name="tarif_pak[]" class="form-control" onkeyup="typingTarifPak(event)" value="{{ $item->tarif_pak }}"></td>
                                              <td><input type="text" name="jumlah_pak[]" class="form-control" readonly value="{{ ($item->volume_pak * $item->tarif_pak) }}"></td>
...
@section('js')
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#form-rba').on('submit', function (e) {
       ...
       var formData = $(this).serialize();
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.rba32.update_pak', $rba->id) }}",
          data: formData,
          success:function(response){
            iziToast.success({
              title: 'Sukses!',
        ...
        rekening.forEach(function (item) {
          data.push(...[
            ...
            },
            {
              ...
              'Tarif Murni': '<input type="text" name="tarif[]" class="form-control" onkeyup="typingTarif(event)" value="0">',
              ...
              'Tarif PAK': '<input type="text" name="tarif_pak[]" class="form-control" onkeyup="typingTarifPak(event)" value="0">',
      ...
      $('.table-rba tbody').on('click', '.btn-add', function () {
        ...
        let element = `
            <tr class="text-dark">
              ...
              <td>
                <input type="text" name="volume[]" class="form-control" readonly onkeyup="typingVolume(event)" value="0">
              </td>
              <td>
                <input type="text" name="satuan[]" class="form-control" readonly>
              </td>
              <td>
                <input type="text" name="tarif[]" class="form-control" readonly onkeyup="typingTarif(event)" value="0">
              </td>
              <td>
              ...
              <td>
                <input type="text" name="tarif_pak[]" class="form-control" onkeyup="typingTarifPak(event)" value="0">
              </td>
              <td>
          ...
          rows.each(function () {
            if ($(this).find('td button').hasClass('parent-item')) {
              ...
              $('.table-rba tbody').find(`input[name="kode_rekening[]"][value="${kodeRekening}"]`).each(function () {
                let jumlah = parseInt($(this).closest('tr').find('td input[name="jumlah_pak[]"]').val());
    ...
    function typingVolumePak(event) {
      let tr = $(event.srcElement).closest('tr');
      let volume = parseInt($(event.srcElement).val());
      let tarif = parseInt($(tr).children('td').find('input[name="tarif_pak[]"]').val());
      let jumlah = volume * tarif;
      ...
      if (! Number.isInteger(volume)) {
        ...
        $(tr).children('td').find('input[name="jumlah_pak[]"]').val(jumlah);
      }
    }
