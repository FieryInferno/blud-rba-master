...
<div class="main-content">
  <section class="section">
      <div class="section-header">
          <h1>RKA 2.1 - {{ auth()->user()->status }}</h1>
      </div>
          ...
          <div class="row">
            @if (auth()->user()->status == 'MURNI')
              @if (auth()->user()->hasRole('Admin'))
                <a class="btn btn-primary mb-4 ml-3" href="{{ route('admin.rka21.create') }}">
                          ...
                          <tbody>
                            @foreach ($rka as $item)
                              <tr>
                                ...
                                <td>
                                  @if (auth()->user()->status == 'MURNI')
                                    <a href="{{ route('admin.rka21.edit', $item->id) }}" class="btn btn-sm btn-primary">
                                      <span class="fa fa-plus"></span> Pilih
                                    </a>
                                  @else
                                    <a href="{{ route('admin.rka21.edit_pak', $item->id) }}" class="btn btn-sm btn-primary">
                                      <span class="fa fa-plus"></span> Pilih
                                    </a>
                                  @endif
...
<form id="form-delete" action="{{ route('admin.rka21.destroy') }}" class="d-none" method="POST">
  @csrf
  @method('DELETE')
