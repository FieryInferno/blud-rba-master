...
<div class="main-content">
  <section class="section">
      ...
      <form action="{{ route('admin.rka1.update_pak', $rka->id) }}" method="POST" id="form-rka">
      @csrf
      @method('PUT')
                                  ...
                                  <tbody>
                                    ...
                                    @foreach ($rka->rincianAnggaran->groupBy('akun_id') as $key => $anggaran)
                                      @foreach ($anggaran as $index => $item)
                                        @if ($index == 0)
                                          <tr>
                                            <td><button type="button" class="btn btn-add btn-sm btn-primary is-parent parent-item d-none"></button></td>
                                          ...
                                          <tr>
                                            <td><button type="button" class="btn btn-remove btn-sm btn-danger d-none"></button></td>
                                            <td><input type="text" name="kode_rekening[]" class="form-control" value="{{ $item->akun->kode_akun }}" readonly></td>
                                            <td><input type="text" name="uraian[]" class="form-control" value="{{ $item->uraian }}" readonly></td>
                                            <td><input type="text" name="volume[]" class="form-control" onkeyup="typingVolume(event)" value="{{ $item->volume }}" readonly></td>
                                            <td><input type="text" name="satuan[]" class="form-control" value="{{ $item->satuan }}" readonly></td>
                                            <td><input type="text" name="tarif[]" class="form-control money" onkeyup="typingTarif(event)" value="{{ $item->tarif }}" readonly></td>
  ...
  <script>
    $(document).ready(function () {
      ...
      $('#form-rka').on('submit', function (e) {
        ...
        $.ajax({
          type: "POST",
          url: "{{ route('admin.rka1.update_pak', $rka->id) }}",
          data: formData,
          beforeSend:function() {
        ...
        rekening.forEach(function (item) {
          data.push(...[
            ...
            },
            {
              ...
              'Jumlah PAK': '<input type="text" name="jumlah[]" class="form-control" readonly>',
