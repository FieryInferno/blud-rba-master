...
<div class="main-content">
  <section class="section">
    ...
    <form action="{{ route('admin.rka21.update_pak', $rka->id) }}" method="POST" id="form-rba">
      @csrf
      @method('PUT')
                            ...
                            <div class="col">
                                ...
                                <select name="unit_kerja" id="unit_kerja" class="form-control">
                                    ...
                                    @foreach ($unitKerja as $item)
                                        <option value="{{ $item->kode }}" {{ ($item->kode == $rka->kode_unit_kerja ? 'selected' : '') }}>{{ $item->kode }} - {{ $item->nama_unit }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                      <div class="row">
                          <div class="col">
                              ...
                              <input type="text" class="form-control" name="sasaran_kegiatan" value="{{ old('sasaran_kegiatan', $rka->kelompok_sasaran) }}">
                          </div>
                      </div>
                  </div>
                  </div>
                                          ...
                                          @foreach ($anggaran as $index => $data)
                                            @if ($data->akun_id == $item->id)
                                              <tr>
                                                ...
                                                <td><input type="text" name="tarif[]" class="form-control money" onkeyup="typingTarif(event)" value="{{ $data->tarif }}" readonly></td>
                                                ...
                                                <td><input type="text" name="tarif_pak[]" class="form-control money" onkeyup="typingTarifPak(event)" value="{{ $data->tarif_pak }}"></td>
                    ...
                    data-nama-rekening="{{ $item->nama_akun }}">
                      <td>
                        @if (!$item->is_parent)
                          ...
                          {{ in_array($item->kode_akun, $allKodeAkunRka) ? 'checked' : '' }}>
                        @endif
                      </td>
                      <td>{{ $item->kode_akun }}</td>
                      <td>{{ $item->nama_akun }}</td>
...
@section('js')
  ...
  <script>
    $(document).ready(function () {
      initMaskMoney();
      getKegiatan('{{ $rka->kode_unit_kerja }}', '{{ $rka->pejabat_id }}');
      ...
      $('#form-rba').on('submit', function (e) {
       ...
       var formData = $(this).serialize();
        $.ajax({
          type: "PUT",
          url: "{{ route('admin.rka221.update_pak', $rka->id) }}",
          data: formData,
          success:function(response){
            iziToast.success({
              title: 'Sukses!',
      ...
      //                             }).join('')}
      //                           </select>`,
      //                 'Volume Murni': '<input type="text" name="volume[]" class="form-control" readonly onkeyup="typingVolume(event)" value="0">',
      //                 'Satuan Murni': '<input type="text" name="satuan[]" class="form-control" readonly>',
      //                 'Tarif Murni': '<input type="text" name="tarif[]" class="form-control money" readonly onkeyup="typingTarif(event)" value="0">',
            ...
            success: function(response) {
              let element = `
                  <tr class="text-dark">
                    ...
                    <td>
                      <input type="text" name="tarif[]" class="form-control" onkeyup="typingTarif(event)" value="0" readonly>
                    </td>
                    <td>
    ...
    function getKegiatan(unit_kerja, id){
       $.ajax({
            ...
            type: "POST",
            data: "kode_unit_kerja="+unit_kerja+"&kode=rka&id="+"{{ $rka->id }}",
            success:function(response){
                ...
                $(response.data).each(function () {
                    if (id){
                      $("<option />", {
                          ...
                          selected: this.id == '{{ $rka->map_kegiatan_id }}' ? true : false
