                              ...
                              <tr>
                                ...
                                <td>
                                  <button class="btn btn-sm btn-warning btn-edit"
                                    ...
                                    data-program="{{ $item->kode_program }}"
                                    data-kode="{{ $item->kode }}"
      ...
      <form action="{{ route('admin.kegiatan.store') }}" method="POST">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select name="kode_program" class="form-control">
                <option value="" selected disabled>-- Pilih Program --</option>
                @foreach ($program as $item)
                  <option value="{{ $item->kode }}">{{ $item->kode }} - {{ $item->nama_program }}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
...
<div class="modal fade" tabindex="-1" role="dialog" id="editModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Sunting Kegiatan</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ route('admin.kegiatan.update') }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
              <label>Kode Program</label>
              <select name="kode_program" id="edit-program" class="form-control">
                <option value="" selected disabled>-- Pilih Program --</option>
                @foreach ($program as $item)
                  <option value="{{ $item->kode }}">{{ $item->kode }} - {{ $item->nama_program }}</option>
                @endforeach
            ...
            <div class="form-group">
              <label>Nama Kegiatan</label>
              <input type="text" id="edit-nama" name="nama_kegiatan" class="form-control" value="" required>
            </div>
        </div>
        <div class="modal-footer bg-whitesmoke br">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
